...
class ProductEditPage extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.match.params.mode === "edit") {
      axios
        .get("http://backend:3100/products/" + this.props.match.params.id)
  ...
  editProductHandler = (event) => {
    ...
    if (this.props.match.params.mode === "edit") {
      request = axios.patch(
        "http://backend:3100/products/" + this.props.match.params.id,
        productData,
      );
    } else {
      request = axios.post("http://backend:3100/products", productData);
    }
    request
      .then((result) => {
        this.setState({ isLoading: false });
